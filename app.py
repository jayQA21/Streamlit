...
.dash-card { background: rgba(15,22,41,0.9); border: 1px solid rgba(0,212,255,0.1); border-radius: 16px; padding: 20px 22px; margin-bottom: 12px; }
.ticket-row { display: flex; align-items: center; gap: 8px; padding: 7px 10px; border-radius: 7px; margin-bottom: 3px; background: rgba(255,255,255,0.015); border: 1px solid rgba(255,255,255,0.05); font-size: 13px; }
...
.stButton > button { background: linear-gradient(135deg, rgba(0,212,255,0.1), rgba(129,140,248,0.1)) !important; border: 1px solid rgba(0,212,255,0.3) !important; color: #00d4ff !important; border-radius: 10px !important; font-weight: 700 !important; font-family: 'DM Sans', sans-serif !important; }
...
def fetch_jira_tickets():
    ...
    for issue in data.get("issues", []):
        ...
        # FIX: convert SP to int to avoid 249.0 display
...
def post_to_slack(blocked_tickets, m):
    ...
    lines = [f"• `{t['key']}` — {t['summary'][:60]}... _({t['assignee'].split()[0]})_"
             if len(t['summary']) > 60
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;flex-wrap:wrap;gap:12px;">
        <div>
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                ...
                <span style="font-size:10px;color:#10b981;text-transform:uppercase;letter-spacing:2px;font-weight:600;">Live · Jira · Refreshes every 5 min</span>
            </div>
            ...
            <div style="font-size:12px;color:#334155;margin-top:4px;">{SPRINT_NAME} · Feb 24 – Apr 12, 2026 · Fetched {fetched_at}</div>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <div style="background:rgba(0,212,255,0.07);border:1px solid rgba(0,212,255,0.18);border-radius:10px;padding:10px 16px;font-size:12px;color:#7dd3fc;text-align:center;">
                📅 Day <strong>{m["current_day"]}</strong> / {SPRINT_DAYS}<br><span style="color:#475569;font-size:10px;">{days_left} days left</span>
            </div>
            <div style="background:{status_color}12;border:1px solid {status_color}35;border-radius:10px;padding:10px 16px;font-size:12px;color:{status_color};text-align:center;">
                {status_label}<br><span style="color:#475569;font-size:10px;">{pct}% done</span>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_burndown(m):
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df["label"], y=df["ideal"], name="Ideal Remaining", mode="lines+markers",
...
def render_velocity(m):
    ...
    for i, (name, d) in enumerate(devs):
        ...
        pct = round((d["done"] / d["total"]) * 100) if d["total"] else 0
        initials = "".join(p[0] for p in name.split()[:2])
        done_w = round(d['done']/d['total']*100) if d['total'] else 0
        active_w = round(d['active']/d['total']*100) if d['total'] else 0
        blocked_w = round(d['blocked']/d['total']*100) if d['total'] else 0
        with dev_cols[i]:
            st.markdown(f"""
            <div style="background:rgba(15,22,41,0.9);border:1px solid {color}28;border-radius:14px;padding:14px;">
                ...
                <div style="background:#1e2d47;border-radius:999px;height:5px;margin-bottom:8px;overflow:hidden;display:flex;">
                    <div style="width:{done_w}%;background:#10b981;"></div>
                    <div style="width:{active_w}%;background:#38bdf8;"></div>
                    <div style="width:{blocked_w}%;background:#f87171;"></div>
                </div>
                <div style="display:flex;gap:4px;flex-wrap:wrap;">
...
def render_tickets(m):
    ...
    for status in STATUS_ORDER:
        ...
        for t in group:
            ...
            assignee_first = t["assignee"].split()[0]
            sp_badge = f'<span style="font-size:9px;color:#fb923c;font-weight:800;font-family:Space Mono,monospace;background:rgba(251,146,60,0.1);padding:2px 5px;border-radius:3px;">{t["sp"]}sp</span>' if t["sp"] else ""
            ...
            <div class="{row_cls}">
                ...
                <span style="font-size:9px;color:#7dd3fc;background:rgba(129,140,248,0.1);border-radius:3px;padding:2px 5px;min-width:44px;text-align:center;flex-shrink:0;">{t['type'][:7] if t['type'] else ''}</span>
                ...
                <span style="font-size:9px;color:{color};font-weight:700;background:{color}12;padding:2px 7px;border-radius:3px;white-space:nowrap;flex-shrink:0;">{initials} {assignee_first}</span>
                {sp_badge}
            </div>"""
        st.markdown(f"""
        <div style="margin-bottom:14px;">
            <div style="font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:5px;color:{sc};display:flex;align-items:center;gap:5px;">
...
def main():
    ...
    with st.spinner(""):
        ...
        <div style="text-align:center;padding:60px 20px;">
            ...
            <h2 style="background:linear-gradient(90deg,#00d4ff,#818cf8,#f472b6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:24px;font-weight:900;margin-bottom:8px;">Fetching Sprint Data</h2>
